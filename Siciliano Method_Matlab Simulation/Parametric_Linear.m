...
[path2, vel2, accel2] = ParametricPath_Func(center, Radius, 4*pi/T, T, 'eightshape');
...
% plot
figure(1);
plot(path.x(tspan), path.y(tspan), '+'); title('trajectory'); hold on; plot(path2.x(tspan), path2.y(tspan), '+'); hold off;
figure(2);
subplot(1,2,1);
plot(tspan,vel.x(tspan)); hold on; plot(tspan,vel.y(tspan)); hold off;
...
figure(3);
subplot(1,2,1);
plot(tspan,accel.x(tspan)); hold on; plot(tspan,accel.y(tspan)); hold off;
...
qd0 = [path.x(0) path.y(0) path.theta(0)]';
error(:,1) = R * (qd0 - x0);
...
%    X(:,i+1) = X(:,i) + dt * ([cos(theta) sin(theta) 0]'*v(i) + [0 0 1]'*w(i)); 
   ...
   R= [cos(theta) sin(theta) 0; -sin(theta) cos(theta) 0; 0 0 1];
   % desired state
   qd = [path.x(t_next) path.y(t_next) path.theta(t_next)]';
   % error dynamics
